...
public class ParamNameResolver {
    ...
    public ParamNameResolver(Configuration config, Method method) {
        ...
        for (int paramIndex = 0; paramIndex < paramCount; paramIndex++) {
            ...
            for (Annotation annotation : paramAnnotations[paramIndex]) {
                //如果参数有Param注解，将Param的value作为形参的名字保存，用于后续的sql解析时的变量替换
            ...
            if (name == null) {
                ...
                if (config.isUseActualParamName()) {
                    // 通过反射获取参数名称。todo 此种方式要求 JDK 版本为 1.8+，且要求编译时加入 -parameters 参数
                ...
                if (name == null) {
                    ...
                    //因为如果参数列表中包含 RowBounds 或 ResultHandler，这两个 参数会被忽略掉，这样将导致名称不连续。
                    //比如参数列表 (int p1, int p2, RowBounds rb, int p3) -
                    // 期望得到名称列表为 ["0", "1", "2"]. 实际得到名称列表为 ["0", "1", "3"]
    ...
    public Object getNamedParams(Object[] args) {
        ...
        if (args == null || paramCount == 0) {
            ...
            /*
            如果方法参数列表无 @Param 注解，且仅有一个非特别参数，则返回该 参数的值。比如如下方法:
